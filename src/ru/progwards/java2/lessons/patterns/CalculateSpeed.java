...
public class CalculateSpeed implements CalcSpeed {
GPS priviosGPS = null;

    @Override
    public Double getSpeed(GPS gps) {
        if (this.priviosGPS == null) {
            this.priviosGPS = gps;
            return 0D;
        }
        else {
            double speed = calculateSpeed(priviosGPS, gps);
            priviosGPS = gps;
            return speed;
        }
    }
    ...
    public String printSpeed(GPS gps){
        ...
        return "Скорость объекта: " + speed + " м/с";
    }

    public double calculateSpeed(GPS gps1, GPS gps2) {
